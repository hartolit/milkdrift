...
`milkdrift-peer-protocol` owns only v1.0 bounded transport-neutral session, catalog, invocation, originating execution provenance, observation, cancellation, delegation, and artifact-transfer messages. `milkdrift-peer-http` owns configured HTTPS/loopback transport, relationship authentication, the fixed dispatch-worker owner, resumable observations, core artifact-transfer adaptation, and the ordinary remote `CapabilityAdapter`. Narrow persistence ports own peer execution semantics and the redb adapter implements them; no HTTP/TLS type enters those contracts.
...
Actor identity and scoped authority are distinct from revision authorship. Canonical `ActorRef` ownership is in `milkdrift-authority`; blueprint `AuthorRef` remains provenance only. Immutable schema-v2 grant revisions constrain commands and reads by workflow/run, typed operation, capability identity/category/operation/profile/trust/locality, normalized filesystem roots and access, credential-free network profiles/destinations, opaque secret references, exact artifact identity/sensitivity, layout revision/owner, peer identity, daemon diagnostic class, workspace scope, side-effect class, cost, duration, invocation, artifact, concurrency, validity, and revocation generation. Empty artifact, layout, peer, workspace, or explicit capability-deny scopes grant nothing. Pure policy evaluation records the actor, exact grant revision, evaluator policy/version, request facts, stable reason codes, evaluated constraints, caller-supplied boundary time, result, and deterministic digest.
...
Portable documents use explicit numeric schema versions: blueprint revision/mutation, invocation request, the context-manifest body, local-process profiles, and prompt-sequence imports are currently v2; daemon configuration is v4; authority grants and external authenticated cursors are v2; the external control protocol is 2.0; CLI JSON output, layout documents, durable application receipt/layout/peer-execution records, the model document envelope, other capability documents, model-task/response contracts, and endpoint profiles are schema 1. Redb physical schema 5 and internal document format 9 are exact-current and deliberately refuse older or future stores without migration. Obsolete file-per-peer-execution and parallel peer-artifact directories are explicitly refused. Daemon configuration v3 is refused because peer artifact transfer did not name exact sensitivity classes. Prompt-sequence v1 is refused because its checkpoint, stage-budget, and profile shapes were not all executable. Context-manifest v1 is retained only as a named legacy constant and refused because it cannot prove selected content or exact producer provenance. Local-process profile v1 is likewise retained only for explicit refusal because its path-only executable reference cannot prove implementation identity. Digest inputs use recursively key-sorted deterministic JSON and deterministic collections. Unknown core variants, malformed typed identities, invalid derived fields, and unsupported future versions fail clearly. Explicit bounded DNS-namespaced extension maps are the only forward-compatible unknown field mechanism.
...
| External control protocol | `milkdrift-control-protocol` owns protocol 2.0 common envelopes/cursors/codecs with focused private `command`, `read`, and `layout` modules for mutation DTOs, observation/read DTOs, and layout schema 1; it contains no async, HTTP, runtime, or storage types |
