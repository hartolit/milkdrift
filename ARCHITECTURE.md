...
| `adapters/redb` | The transactional local adapter, split across `milkdrift-redb-store::{admin,journal,store}` facades and their private child modules; `peer::integrity` owns whole-store peer ownership, index, observation-chain, and accounting verification |
