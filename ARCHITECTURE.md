...
Portable documents use explicit numeric schema versions: blueprint revision/mutation and invocation request are currently v2; daemon configuration is v3; authority grants, external authenticated cursors, and the daemon control sidecar are v2; the external control protocol is 1.0; CLI JSON output and layout documents are schema 1; other capability documents, context manifests, model contracts, and endpoint profiles are v1. Digest inputs use recursively key-sorted deterministic JSON and deterministic collections. Unknown core variants, malformed typed identities, invalid derived fields, and unsupported future versions fail clearly. Explicit bounded DNS-namespaced extension maps are the only forward-compatible unknown field mechanism.
