...
| `adapters/redb` | The transactional local adapter, split across `milkdrift-redb-store::{admin,journal,store}` facades and their private child modules; one `admin::service::integrity_scan` state drives typed integrity phases under one read transaction and refusal policy, while `peer::{accounting,integrity,retention,validation}` owns durable peer accounting, whole-store verification, archival compaction, and document validation around the cohesive execution-store implementation |
...
| `apps/daemon` | `milkdrift-daemon::{config,auth,host,http}` owns validated local/peer configuration, credential-to-actor/peer mapping, redb/runtime/capability/effect/peer lifecycles, the bounded owner boundary, separated control/peer HTTP realms, readiness, and ordered shutdown; private `host::commands` families preserve one authorized command route, private `host::attempts` phases preserve one current/historical attempt meaning, and `host::read_model` owns external DTO/error projection |
