...
| `apps/cli` | `milkdrift-cli` is a thin argument/presentation layer over `milkdrift-control-client`; its root owns arguments/composition, private command-family modules own presentation routing, and one private session owner centralizes credentials, envelopes, confirmation, bounded input/output, downloads, and exit/error mapping; it never owns durable truth |
