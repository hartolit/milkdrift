...
The headless runtime owns transition decisions, projections, leases, scheduling, recovery, and reconciliation through narrow persistence/execution ports. `TaskExecutor` has one execution path: caller-owned workers submit each observation through the incremental durable reporter, with no parallel synchronous batch result or validation path. `milkdrift-capability-host::EffectWorkerHost` is the explicit embeddable owner of bounded caller-created effect and cancellation workers; it has no singleton or async runtime. The daemon owns that object together with registry, secret, redb, artifact, recovery, and shutdown lifecycles. Clients request commands and render projections; they do not own truth.
...
New resolved-capability snapshots freeze and digest the descriptor category used for controller process/model accounting. Existing schema-1 snapshots written before that additive fact remain replayable under their original digest for ordinary execution; a marked controller history without an exact account declaration, binding, and frozen category fails closed rather than reconstructing resource truth.

`milkdrift-control::ControllerLifecycleOwner` is the one controller-policy parser and lifecycle assessor for focused library integrations and tests. It derives an immutable account declaration from controller run, controller execution, policy digest, exact currency, and the six ledger-owned ceilings. `milkdrift-persistence` owns the validated account state, reservations, optimistic revision digest, and run binding; `milkdrift-redb-store` mutates them only inside the existing journal or artifact transaction. Controller runs and descendants inherit one exact immutable binding at durable child creation. Nested controller policies inside an already bound descendant are refused.

The sole runtime final-adapter-entry path re-evaluates authority, prepares one exact capability generation and host permit without entering adapter code, obtains its request-specific admission envelope, and atomically commits the canonical adapter-entry event with the corresponding account admission transition. Process/model counts come from the frozen descriptor category. Bounded unit, cost, and artifact obligations reserve before entry; `Unknown`, currency mismatch, blocked state, overflow, or a ceiling violation is durably denied before adapter execution. Terminal evidence settles only authoritative use, missing bounded observations retain their remainder and block future admission, uncertain outcomes retain every remainder, and over-envelope evidence durably blocks the account. Logical artifact publication charges the exact invocation reservation or the run binding in the artifact metadata transaction; replay, content deduplication, and abort do not double-charge. Controller assessment consumes settled totals plus all outstanding remainders while retaining lifecycle ownership of cycles, revisions, proposals, elapsed time, failures, rejections, and depth. The production daemon still leaves this lifecycle uninstalled until a current qualifying real external-evidence run satisfies the activation gate.
...
`host::clock` owns one fallible system-clock source and adapts it into the runtime, peer, control,
artifact, and stream boundaries. Every daemon observation crosses the owner queue and advances one
redb high-water fact before it can authorize, expire, schedule, or timestamp work; artifact
acceptance advances the same fact in its write transaction. Clock failure or an observation behind
durable high-water evidence refuses the affected operation, records redacted health/log evidence,
and prevents readiness when found during startup. A peer worker that loses time after durable
adapter entry retains the exact recovery record and retries uncertainty persistence without a
second adapter invocation. The operating-system wall clock remains the explicit source of elapsed
time while the daemon is not running; durable high-water evidence prevents a restart or downgrade
from forgetting any later instant that Milkdrift already observed. During shutdown the daemon
temporarily transfers the effect-worker owner to a blocking shutdown task while the runtime owner
continues servicing final clock and persistence requests, then stops the owner thread only after
the bounded worker result returns.
...
releases occupancy on dequeue or drop. A request panic closes ordinary admission and records failed
health; the owner remains available for final worker persistence, clock and shutdown calls, skips
normal maintenance, and cannot report a clean shutdown. Queued ordinary requests recheck readiness
before executing. These facts govern only local admission and observability;
they are not workflow, execution-history, or storage truth.

`milkdrift-control-protocol` owns pure external protocol 2.3 DTOs. Protocol 2 makes the frozen run authority basis and resolution/claim/final-entry decisions part of every available attempt inspection; minor 1 adds redacted application-receipt lifecycle health, minor 2 adds durable attempt output/progress/usage plus exact redacted process/model provenance, and minor 3 adds exact capability/attempt external-effect contracts, idempotency-key presence, and safe timeline classification. Closed protocol-1 clients are deliberately rejected rather than receiving an unversioned shape change. Its read models project immutable revisions, compact runs/nodes/attempts, proposal state, timelines, capabilities, authority, artifacts, peers, and health without serializing internal event variants, command/result bytes, or redb keys. Authenticated cursor schema 2 binds feed position to actor, exact grant identity/revision/digest, decision digest, query resource/filter digest, and a credential-derived MAC. A continuation cannot cross actor, credential rotation, grant replacement, or query scope. Streams authorize establishment, use the same bound cursor, and reauthenticate and reevaluate at each bounded polling cycle; revocation stops future disclosure and is distinguishable from transport failure. `milkdrift-control-client` is the only HTTP mapping used by the CLI and any separately authorized external client. The CLI cannot create hidden state or directly mutate journals.
...
lexical validation, the common validated-string newtype implementation, and the
mechanical adapter from a private Serde wire shape to its owner's validated conversion.
Domain crates continue to own their wire shapes, identities, digest domains, validation
...
Portable documents use explicit numeric schema versions: blueprint revision/mutation, invocation request, the context-manifest body, local-process profiles, prompt-sequence imports are currently v2; durable hot peer-execution records are v3; run-event envelopes are v3; resolved-capability snapshots are v2; authority decisions are v2; daemon configuration is v9; authority grants are v4; external authenticated cursors are v2; the external control protocol is 2.3 and peer protocol is 1.2; CLI JSON output is schema 2; layout documents, durable application receipts/layouts/peer-execution tombstones, the model document envelope, other capability documents, model-task/response contracts, and endpoint profiles are schema 1. Exact run-event v1/v2 readers remain supported while the writer emits v3 controller-account meaning. Redb physical schema 11 and internal document format 14 are exact-current and deliberately refuse older or future stores without migration. Physical schema 9 introduced the durable boundary-clock high-water fact; schema 10 added controller accounts, immutable run bindings, transition evidence, and artifact-charge links; schema 11 adds immutable per-account revision evidence. Internal format 13 distinguished not-applicable reservations from settled dimensions and retained self-validating controller transitions linked to their atomic command receipts; format 14 links every controller-account mutation to a replayable predecessor plus its exact transition action or artifact publication. Artifact authority uses an explicit whole-scope `DenyAll` or a nonempty `Any`/bounded-`Only` identity selector paired with nonempty sensitivities; layout authority uses `DenyAll` or explicit shared-layout revision selectors. Empty collections never mean wildcard, and actor-owned layouts remain reserved rather than advertised as executable daemon state. Obsolete file-per-peer-execution and parallel peer-artifact directories are explicitly refused. Daemon configuration v8 and earlier is refused: v8 was the superseded JSON/global-document boundary, and earlier selector intent also cannot be recovered safely. Prompt-sequence v1 is refused because its checkpoint, stage-budget, and profile shapes were not all executable. Context-manifest v1 is refused because it cannot prove selected content or exact producer provenance. Local-process profile v1 is refused because its path-only executable reference cannot prove implementation identity. Digest inputs use recursively key-sorted deterministic JSON and deterministic collections. Unknown core variants, malformed typed identities, invalid derived fields, and unsupported future versions fail clearly. Explicit bounded DNS-namespaced extension maps are the only forward-compatible unknown field mechanism.
...
| --- | --- |
| Shared contract mechanics | `milkdrift-contracts` owns bounded/canonical JSON mechanics, UTF-8-safe byte truncation, canonical `b3_` lexical validation, common validated-string implementation, and private-wire-to-validating-conversion glue; wire shapes and semantic rules remain in consuming domain crates |
...
| External control protocol | `milkdrift-control-protocol` owns protocol 2.3 common envelopes/cursors/codecs with focused private `command`, `read`, and `layout` modules for mutation DTOs, observation/read DTOs, and layout schema 1; it contains no async, HTTP, runtime, or storage types |
...
| `workspace/context` | Immutable task policy/output roles in `milkdrift-blueprint::context`, exact schema-v2 manifest contracts in `milkdrift-model::context`, and runtime-owned construction in `milkdrift-runtime::context`; private discovery phases seed projections, fold bounded journal pages, classify events, complete explicit sources, and expose joins/subworkflows, while a separate private selection state owns authority/sensitivity/budget checks, deterministic ordering, omissions, and final publication |
...
| `persistence/events` | `milkdrift-persistence::{event::{model,kind,references},document}` owns durable facts and strict readers; private reference validators separate lifecycle, continuation, execution, structured-work and reconciliation invariants around one bounded run context, with exact legacy and current golden fixtures |
| `persistence/journal` | Narrow receipt/atomic-commit/query ports in `milkdrift-persistence::journal::{receipt,commit,query}`, implemented by `milkdrift-redb-store::journal::{append,discovery,queries,workspace}`; workspace writes retain provenance and accounting in one journal transaction, while `workspace::reads` owns snapshot queries |
| `persistence/controller-account` | One public persistence contract with private `controller_account::{identity,transaction,transition,validation}` owners; redb `controller_account::validation` checks event and lineage applicability before journal/artifact transactions mutate the ledger |
...
| `adapters/redb` | `schema::physical_tables` declares every physical name and type once for initialization and exact membership validation; `store::schema` owns metadata, version checks and commit faults. `admin::scan` owns bounded physical iteration and `admin::service::integrity_scan` drives semantic phases under one read transaction. `peer::{claims,records,accounting,integrity,retention,validation}` separates complete claim transactions, validated hot/archive reads, accounting, integrity and retention. `application` keeps receipt-plus-effect commits atomic; private `layouts`, `proposals` and `audit` children own their distinct reads, validation and transactions |
...
| `apps/daemon` | `config::{wire,compile,redaction}` turns strict input into immutable owner plans. `host::{startup,queue,authorization,peers,maintenance,shutdown}` owns construction, the sole bounded owner thread, boundary authority, peer composition, maintenance and ordered drain; command/read families use that owner and `read_model` projects public DTOs. `http::{commands,reads,peers,streams,artifacts,response}` owns route-family adaptation and public errors without durable or runtime authority |
| `apps/cli` | `milkdrift-cli` is a thin argument/presentation layer over `milkdrift-control-client`; its root owns arguments/composition, private command-family modules own presentation routing, and private session, input, output, streaming and error owners keep credentials, envelopes, bounded I/O, confirmation and rendering separate; it never owns durable truth |
