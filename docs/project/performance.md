...
CI target size/free-space observations are infrastructure evidence, not product performance. The Quality topology compiles `compile.maintained-benches` in its own fresh target and gives check, tests, Clippy, and rustdoc separate standard-runner filesystems; none of their artifact sizes is a runtime-memory, throughput, or latency measurement. Resource procedures and current workflow boundaries are documented in [validation](validation.md#shared-hosted-quality), while useful exact-tree chronology remains in [execution history](../agent/execution/history.md).
