...
The Candle package is the deterministic owner for required-versus-complete-observed scalar policy, all declaration states, every Safetensors 0.8 category, required unsupported-dtype rejection, ignored-extra non-materialization/transfer, exact required-weight and bounded logical CPU/CUDA load footprints, supplied-expectation versus locally established verification state, same-inode/path mutation handling, bounded structural metadata/allocation failure, malformed/duplicate/gapped/overlapping/truncated/reordered shards, and partial-load ownership at every stage. Homogeneous required layouts may accept an absent declaration; mixed required F16/F32 and BF16/F32 fixtures must reject absence and require their matching declaration. Sequence-plan tests separately prove that all-layer persistent KV ownership scales with depth while one block's transient forward peak does not, including the realistic 22-layer TinyLlama configuration. The host-budget test requires loading-peak rejection before materialization. The Hub package owns bounded immutable-commit discovery, strict declaration parsing, shard selection, exact LFS identity evidence, verified Git-blob evidence, and project-established local origins; Candle receives only provider-neutral expected content.

Accelerator unit tests exercise a homogeneous entry, mixed cast staging, an exact byte boundary, the count boundary, an oversized singleton, shard separation, arithmetic overflow, one-byte-low host/device/current-device admission, and equality between the immutable partition consumed by footprint planning and materialization. The fault matrix covers source creation, cast, one or more enqueues, pre-sync, failing sync, post-sync/pre-commit, partial commit, a later batch after earlier commits, a late shard digest mismatch, and handle-only model construction. A populated multi-entry batch owner remains byte/count/coordinate/plan stable after one injected cleanup failure, then releases on retry and treats another cleanup call idempotently.
