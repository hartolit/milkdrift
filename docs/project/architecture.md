...
E1 passes the exact selected domain `ExecutionDevice`; E0 admits the exact required-only loading peak, verifies actual device/execution scalar/final accounted footprint, and publishes only final reserved ownership before loaded state becomes visible. The complete opt-in feature graph remains `desktop-slint/cuda -> application-runtime/cuda -> candle-backend/cuda`; the separate `inference-runtime/cuda` forwarding edge is development-only. No default graph reaches CUDA, and explicit CUDA failure never falls back to CPU. Phase 11 Actions evidence remains historical. The 2026-08-08 Phase 12 closure and the 2026-08-10 artifact-loading amendment each passed the exact local CUDA compile and deterministic hardware matrix on the narrowly identified RTX 5070 Ti row; only the earlier closure ran the complete canonical gate. No GitHub self-hosted workflow, generic NVIDIA support, or external mixed-checkpoint claim follows. GGUF is unsupported; any quantized path requires separate compatibility, tokenizer provenance, immutable identity, quantization, cleanup, and device evidence.
