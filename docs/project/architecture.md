...
E0 `inference-runtime` exclusively owns loaded model generations, backend sequences, request admission, generation workspaces, sampling execution, cancellation boundaries, draining, cleanup quarantine, accounting, unload, and shutdown. Its contracts describe direct ownership of model resources and token-step scheduling. E0 verifies complete loaded descriptors, requested versus actual execution device, planned versus actual execution scalar, and planned versus adapter-reported accounted footprint before publishing a receipt whose reserved footprint records the committed E0 admission quantity. It also verifies advertised limits and capabilities, sequence identity/state, position transitions, and exact vocabulary logits rather than trusting adapter claims after trait conformance. Neither footprint is a physical-memory observation. [ADR-0010](../agent/decisions/0010-verify-backend-contracts-at-e0.md) records the general substitution rule; [ADR-0019](../agent/decisions/0019-explicit-cuda-execution-foundation.md) applies it to devices and CUDA accounting.
...
`ModelSelection` contains only a normalized Hugging Face repository and requested revision. Resolution is device-independent: `ResolvedModel` reports artifacts, source, Safetensors format, validated source scalar, tokenizer, immutable identity, and Llama/Candle compatibility evidence, but no execution scalar or selected/actual device. E1 stores `ApplicationDevice::{Cpu, Cuda { ordinal: u32 }}` separately; after load, `LoadedModel` reports the verified source scalar, receipt-verified execution scalar, and actual device.
...
Execution engine, model format, artifact source, scalar stages, and device are separate concepts rather than interchangeable caller-assembled axes. The reviewed E1 composition is Candle plus immutable Hugging Face Safetensors on mandatory/default CPU or explicit feature-gated CUDA ordinal 0. Product CUDA support is limited to the executed Linux x86_64 RTX 5070 Ti matrix in [implementation status](implementation-status.md); the feature graph is not a generic Linux or NVIDIA support claim. E1 passes the exact selected domain `ExecutionDevice`; E0 verifies actual device, execution scalar, and adapter accounted footprint, then publishes its reserved footprint for E1 validation before loaded state becomes visible. The complete opt-in feature graph is `desktop-slint/cuda -> application-runtime/cuda -> candle-backend/cuda`; the separate `inference-runtime/cuda` forwarding edge is development-only. No default graph reaches CUDA, and explicit CUDA failure never falls back to CPU. GGUF is unsupported; if pursued later, Candle-native GGUF or another quantized format belongs under the Candle execution path and requires separate compatibility, tokenizer-provenance, artifact-identity, quantization, and device evidence.
...
Admission validates capacities and accounting before state becomes visible. E1 re-probes the selected device, passes its exact `ExecutionDevice`, and validates the receipt ticket, logical model identity/handle, immutable resolution/artifacts, source scalar, execution scalar, Llama/Candle/Safetensors evidence, tokenizer vocabulary, selected versus actual device, and E0 reserved footprint. E0 has already verified the backend’s accounted footprint against the accepted plan. Cleanup failure does not imply release: a mismatch publishes no `LoadedModel`, and unresolved E0 resources remain in the existing private incompatible-load quarantine, owned and accounted through retry and exhaustion. A failed load that reports retained E0 cleanup keeps E1 unloading and device selection locked; a bounded private snapshot returns it to idle only after zero aggregate ownership is proven. Successful unload clears actual loaded-device state but preserves application selection.
...
- Immutable Hugging Face Hub Safetensors execute on mandatory/default CPU or explicit opt-in CUDA ordinal 0 on the exact executed Linux x86_64 RTX 5070 Ti matrix; no other CUDA target is claimed.
...
- Product-model CPU/CUDA evidence and download-free CUDA hardware CI are accepted for the exact matrix. Phase 11 and post-Phase 11 quality closure are complete; no subsequent product phase is active.
