...
This pre-release store format refuses physical schema versions other than 10 and internal document formats other than 12. Physical schema 9 introduced the boundary-clock high-water fact, and schema 10 adds controller accounts, immutable run bindings, transition fingerprints, and artifact-charge links; format 12 carries the corresponding exact-current internal documents. Startup also refuses obsolete `peer-executions-v1` or `peer-artifacts-v1` directories instead of silently ignoring or partially importing prototype authority.
