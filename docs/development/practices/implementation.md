...
Use this practice when designing, implementing, refactoring, or reviewing code. It owns general
implementation quality, Rust design, code organization, and adoption of changed designs.
Product intent and system ownership remain with [vision](../../product/vision.md) and
[architecture](../../architecture.md). Use the [workflow](../workflow.md) for assignment scope,
completion, and verification, and the [practice guide](README.md) to select complementary guidance.
...
5. [Apply an implementation change](#5-apply-an-implementation-change)
...
under the [documentation practice](documentation.md); do not delete it, compress its formatting, or relocate it merely to satisfy a
...
## 5. Apply an implementation change

Follow the [assignment procedure](../workflow.md#carry-out-an-assignment). Choose the simplest
complete design and the smallest suitable Rust mechanism.
...
7. Capture the reason for a non-obvious design choice while making it. A local comment should
   explain what a maintainer could otherwise misunderstand; straightforward code can stand alone.
   Keep the intended behavior and contract accurate as the implementation changes.

Once the behavior and structure have settled, review the affected explanation under the
[completion procedure](../workflow.md#before-finishing). Follow changed relationships into the
relevant API docs, package introduction, or guide. A new symbol does not automatically need a new
walkthrough, and clear existing explanations may remain. The [documentation practice](documentation.md)
explains how to develop and review the reader's account of the operation.

Do not finish merely because the requested edit exists. Finish when one clear, complete, and
extensible implementation remains.
