...
Semantic observations are contiguous from sequence one and map to progress, stream, artifact, terminal, or uncertainty. Pages use an exclusive `after_sequence` cursor and bounded limit. SSE uses the same encoded observations plus independent transport keepalive comments; polling is equally resumable. The service reevaluates the exact peer execution scope on every page and bounded stream cycle; credential rotation/revocation emits `authorization_terminated` and stops future disclosure. Terminal closure is explicit. A slow consumer reads bounded pages from the durable log.
