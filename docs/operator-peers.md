...
This pre-release store format refuses physical schema versions other than 5 and internal document formats other than 9. Format 9 persists the lifecycle admission gate and exact adapter-entry authority evidence. Startup also refuses obsolete `peer-executions-v1` or `peer-artifacts-v1` directories instead of silently ignoring or partially importing prototype authority.
