...
## Phase 11 — executed CPU + Linux CUDA closure

- **Prepared:** 2026-08-04
- **Accepted code-under-test (Commit E):** `411945e0fd53363f98609db21a43d757c4d9b506`, tree `7099dcb5c9879190543d3afa5fde399a84d799df`
- **Recorded outcome:** Phase 11 complete for mandatory CPU and the executed Linux x86_64 CUDA ordinal-0 matrix only; no subsequent phase active

Commit E was clean for the accepted execution. CPU remained the mandatory default/shared-CI path and passed the executed CPU tests. CUDA support was accepted only for ordinal 0 on the NVIDIA GeForce RTX 5070 Ti with driver 610.43.03, toolkit 13.3, compute capability 12.0, and build target 120; this is not a generic NVIDIA compatibility claim. The non-default product graph is `desktop-slint/cuda -> application-runtime/cuda -> candle-backend/cuda`, the benchmark graph is `runtime-benchmarks/cuda -> application-runtime/cuda`, the direct E0 test edge is `inference-runtime/cuda -> candle-backend/cuda`, and no default graph reaches CUDA.

The exact supported TinyLlama primary workload passed on CPU and CUDA, including cancellation, release, unload, and shutdown. Three complete CUDA cycles were stable. The direct E0 CUDA snapshot test proved zero model/request/workspace/cleanup accounting; CUDA adapter and E1 tests passed; schema-2 chat timing is now recorded; final CPU and CUDA compile/test/Clippy gates passed; and raw reports remained beneath ignored root `target/`. Exact timing and memory tables are intentionally not duplicated here; see [performance evidence](../../project/performance.md#external-product-evidence).
...
## Phase 12 — segmented per-tensor Safetensors compatibility

- **Activated:** 2026-08-08 under the segmented execution guide
- **Segment 1:** commit `58490fe693fef7a2635956181088664cd90685e8` (`58490fe`)
- **Segment 2:** commit `12510695aa29be6a2665dbf3777cccbb8172c2d1` (`1251069`)
- **Segment 3 / closure:** this coherent validation/project-truth commit

Segment 1 replaced the configuration-wide homogeneous scalar assumption with adapter-owned per-tensor Safetensors inspection and a consumable prepared-load transaction. It implemented exact checked final and loading-peak plans, homogeneous F32/F16/BF16 and mixed F16/F32 or BF16/F32 conversion policy, E0 admission/receipt verification, and explicit failed-load ownership that is either cleaned or retained and accounted through bounded retry/exhaustion.

Segment 2 propagated the corrected semantics through immutable Hugging Face resolution and the current application kit. Configuration-declared scalar metadata became optional producer-intent evidence; E1 retained actual execution scalar/device facts from E0 without reproducing Candle's per-tensor policy; persistence remained backward-readable; cleanup-pending/exhausted load states remained explicit; and Slint received only compile/presentation adaptation with no tensor-inspection or workflow responsibility.

Segment 3 updated project-authored CPU/CUDA fixtures, hosted E0 coverage, deterministic budget and cleanup faults, benchmark evidence, the self-hosted CUDA gate, and canonical documentation. The normal synthetic report now has current schema 3, separating declared, observed, and planned facts, exact final/loading-peak plans, directly observed E0 reserved ownership, and process RSS. The external report independently has current schema 4, separating declared, observed, planned, and actual product facts, exact plan footprints, process RSS, and whole-device CUDA memory while refusing to fabricate same-worker E0 ownership. Historical Phase 10/11 reports and measurements retain their original schemas and meanings.

Before canonical closure, the following focused, download-free CPU commands passed:

- `cargo test --locked -p candle-backend --test llama_cpu`: 20 passed;
- `cargo test --locked -p inference-runtime --test native_backend_generation`: 3 passed, including mixed F16/F32 hosted E0 generation and full release/unload/shutdown lifecycle;
- `cargo test --locked -p inference-runtime --test fault_injection`: 32 passed, including aggregate loading-peak budget rejection and retained/retry/exhausted failed-load cleanup paths;
- `cargo test --locked -p runtime-benchmarks`: 78 passed, including synthetic schema 3 and external schema 4 serialization/separation tests.

Canonical closure then passed on Linux 7.1.5-arch1-2 x86_64 with an AMD Ryzen 9 5950X (16 cores/32 threads), Rust 1.96.1, and Cargo 1.96.1: `cargo xtask verify` from a previously absent Cargo target directory; both `wasm32-unknown-unknown` and `thumbv7em-none-eabihf` checks for all five domain crates; locked cargo-deny policy; offline Lychee with 276 links and 0 errors; and the exact CUDA check/test-compilation/Clippy chain with `CUDA_COMPUTE_CAP=120`.

The complete local hardware matrix passed on 2026-08-08 on NVIDIA GeForce RTX 5070 Ti ordinal 0, driver/KMD 610.43.03, CUDA UMD/toolkit 13.3, `nvcc` 13.3.73, compute capability 12.0, and build cap 120. The explicit CPU-in-CUDA adapter test, all four ignored adapter tests, mixed F16/F32 hosted-E0 lifecycle, all 32 fault tests under the CUDA feature graph, E1 explicit no-fallback, and guarded E1 CUDA fixture lifecycle all passed. At commit-authoring time the updated Phase 12 GitHub self-hosted workflow had not yet run, so no result was inferred from workflow source; its later result is appended below without rewriting that chronology.

Pinned `TinyLlama/TinyLlama-1.1B-Chat-v1.0` at `fe8a4ea1ffedaf415f4da2f062534de366a451e6` is homogeneous BF16 and remains a historical product/lifecycle profile, not mixed-checkpoint evidence. No suitable immutable, license-reviewed external mixed Llama profile was established for Segment 3. Missing network access or credentials would be acquisition failures rather than incompatibility, and neither is being substituted for an executed compatibility result.

Phase 12 was complete at authoring for deterministic CPU compatibility and the exact locally executed CUDA matrix. The then-pending Phase 12 GitHub self-hosted workflow and absent external mixed-checkpoint were explicit evidence gaps; the later workflow fact is appended below, while the checkpoint gap remains. The planned next major program returned to workflow, workspace, artifact, authority, capability, budget, and execution-endpoint foundations rather than continuing open-ended Candle loader expansion.

### Later GitHub evidence for the Phase 12 closure commit

The statements above accurately describe what was known when commit `181a069ce81525e9c144fe8de051ced8e3c0b9d7` was authored. After that commit was pushed on 2026-08-08:
...
- hosted Quality [run 31281013257](https://github.com/hartolit/milkdrift/actions/runs/31281013257) failed after the canonical native `cargo xtask verify` step had succeeded. The old workspace-wide release bench compilation left approximately 49 MiB free, and the subsequent portable check created a second root target and failed with `No space left on device`; the following linker bus error was consequential.

The Quality result is CI-infrastructure history, not evidence of a Rust/WASM product failure. Neither run proves later commits.

## Pristine post-Phase 12 ownership amendments

Three coherent amendments followed the Phase 12 closure:

- `d4a1e4324a6793becc56147e4b2e3246189d2693`, tree `1357ef2bbbe7c7315b92e1374f5ac22aab18c3c6`, hardened declaration parsing, required-versus-complete-observed scalar truth, selective whole-shard materialization, source identity, structural bounds, and required-only footprints. Its targeted native and exact local CUDA matrix passed.
- `b43d0f47953c5319a41340d9087b7fd8f07b3280`, tree `634ebb55dd654767f3751b49f46063925ae548d2`, separated byte ownership from cache rates, exact from unverified retention, and explicit cleanup/admission state. Targeted native and isolated portable checks passed; CUDA compilation stopped before Rust diagnostics because `nvcc` was unavailable in that validation environment.
- `1f91cba691a8099805fa31f576079e79c282c73e`, tree `00b519d288fca5551b15d4d3e8c64c2056c624ca`, tightened E1 retained ownership, persistence `LAM1` v3, and the frontend-neutral application boundary. Its commit contains the current application behavior; earlier Phase 12 Actions evidence is not reused for it.

These amendments did not activate a later product phase or establish an external mixed checkpoint.

## Infrastructure-truth maintenance closure

**Prepared:** 2026-08-10
**Input baseline:** `1f91cba691a8099805fa31f576079e79c282c73e`, tree `00b519d288fca5551b15d4d3e8c64c2056c624ca`
**Recorded outcome at this revision:** implementation and every locally available acceptance gate complete; CUDA/tooling prerequisites and post-push evidence remain explicit; the resulting coherent commit is reported externally

This maintenance package replaces package-name/exact-edge registries with explicit manifest roles, a generic inward role DAG, the actual acyclic domain graph, exact development/external/CUDA exceptions, and declarative maintained benchmark targets. The canonical gate compiles only `runtime-benchmarks/runtime` and `sampling/sampling_pipeline` as release benchmarks.

Hosted quality work is split into native, portable matrix, policy, nursery, and link jobs with named isolated targets, free-space preflight, disk observations, root-target leak checks, and unconditional cleanup. CUDA uses separate check and release-hardware targets and three dedicated harness-free suite boundaries; workflow correctness no longer parses test names. First-party checkout uses immutable v7.0.1 commit `3d3c42e5aac5ba805825da76410c181273ba90b1`.

External evidence schema 6 removes the independent observer preparation and records only public E1 product facts plus raw timing/process/qualified whole-device observations. Historical schema values remain unchanged, and no schema-6 product run is claimed.

Final available local acceptance passed:

- `cargo test --locked -p xtask`: 3 unit, 21 architecture, 8 hygiene, and 1 command-surface tests, including executable harness-free CUDA fixture targets; direct architecture and hygiene commands also passed;
- a fresh shim-guarded `cargo xtask verify`, including only the two exact maintained release benchmark targets;
- fresh WASM and thumb checks for all five portable domain packages;
- pinned cargo-deny 0.20.2 advisories, bans, licenses, and sources policy;
- pinned Lychee 0.24.2 offline repository-link validation; and
- final formatting and whitespace checks.
