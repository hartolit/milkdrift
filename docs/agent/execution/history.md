...
The complete local hardware matrix passed on 2026-08-08 on NVIDIA GeForce RTX 5070 Ti ordinal 0, driver/KMD 610.43.03, CUDA UMD/toolkit 13.3, `nvcc` 13.3.73, compute capability 12.0, and build cap 120. The explicit CPU-in-CUDA adapter test, all four ignored adapter tests, mixed F16/F32 hosted-E0 lifecycle, all 32 fault tests under the CUDA feature graph, E1 explicit no-fallback, and guarded E1 CUDA fixture lifecycle all passed. The updated Phase 12 GitHub self-hosted workflow has not run; no result is inferred from workflow source.
...
Phase 12 is complete for deterministic CPU compatibility and the exact locally executed CUDA matrix. The unrun Phase 12 GitHub self-hosted workflow and absent external mixed-checkpoint remain explicit evidence gaps. The planned next major program returns to workflow, workspace, artifact, authority, capability, budget, and execution-endpoint foundations rather than continuing open-ended Candle loader expansion.
