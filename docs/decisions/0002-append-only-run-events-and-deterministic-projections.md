...
Ownership is explicit: the journal is complete immutable history; the active projection is bounded operational state; a snapshot is an optional bounded recovery checkpoint; historical read models reconstruct or query paged journal facts. Projection snapshot schema v3 names its exact covered sequence and cumulative history digest and serializes the live-compacted projection without first cloning a historical projection. Invalid, mismatched, corrupt, v1, v2, or otherwise unsupported optional checkpoints are discarded and replayed rather than guessed or migrated.
