...
Ownership is explicit: the journal is complete immutable history; the active projection is bounded operational state; a snapshot is an optional bounded recovery checkpoint; historical read models reconstruct or query paged journal facts. Snapshot persistence envelope v2 names its exact covered sequence and cumulative history digest and contains the independently versioned runtime projection payload v4, serialized from the live-compacted projection without first cloning a historical projection. Payload v4 adds the exact terminal-sequence anchor required for bounded causal evidence lookup from settled summaries. When the runtime chooses a checkpoint boundary, a domain-separated commitment to those exact payload bytes is stored atomically in the history-chain record for the accepted resulting head. Storage verifies both the event-prefix digest and that append-time payload commitment before returning a snapshot as verified, so an independently checksummed but semantically divergent payload cannot become a second authority. Invalid, mismatched, corrupt, envelope-v1, payload-v1/v2/v3, or otherwise unsupported optional checkpoints are discarded and replayed rather than guessed or migrated.
