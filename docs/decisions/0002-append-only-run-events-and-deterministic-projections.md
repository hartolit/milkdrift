...
Ownership is explicit: the journal is complete immutable history; the active projection is bounded operational state; a snapshot is an optional bounded recovery checkpoint; historical read models reconstruct or query paged journal facts. Snapshot persistence envelope v2 names its exact covered sequence and cumulative history digest and contains the independently versioned runtime projection payload v5, serialized from the live-compacted projection without first cloning a historical projection. Payload v5 retains the accepted agreement and cumulative adoption count in addition to the exact terminal-sequence anchor required for bounded causal evidence lookup from settled summaries. When the runtime chooses a checkpoint boundary, a domain-separated commitment to those exact payload bytes is stored atomically in the history-chain record for the accepted resulting head. Storage verifies both the event-prefix digest and that append-time payload commitment before returning a snapshot as verified, so an independently checksummed but semantically divergent payload cannot become a second authority. Invalid, mismatched, corrupt, envelope-v1, payload-v1/v2/v3/v4, or otherwise unsupported optional checkpoints are discarded and replayed rather than guessed or migrated.
...
Commands can be retried without duplicating semantic facts, recovery has one sequence authority, and historical read models can be rebuilt independently. Command validation and candidate construction clone only the bounded active projection where atomic validation needs a disposable candidate. Event evolution requires explicit migration and golden fixtures. Run-event schema 2 introduced controller assessments, actor-attributed revision adoption, and exact subworkflow usage under a new checksum domain; readers retain exact schema-1 fixtures and reject schema-2 meaning disguised as v1. More facts are recorded, but large content remains in the artifact store and events carry bounded references.

This does not promise universal constant memory. Active branches, unresolved or retained effects, current output/context/artifact references, workflow shape, and explicit workspace limits remain in scope. Full execution occurrences remain only through their legal transition and safety obligations; closed occurrences collapse to one current scope/node terminal summary, replacing any older summary for that semantic frontier. Settled retry attempts collapse to a total-attempt count plus the last identity/facts required by the current occurrence. Completed branch/join ownership, iteration scopes, and imported child-workflow links retire after their consumers close. Recovery, continuation, progress, lease, timer, signal, superseded execution, and reconciliation histories remain in the journal.
