...
Ownership is explicit: the journal is complete immutable history; the active projection is bounded operational state; a snapshot is an optional bounded recovery checkpoint; historical read models reconstruct or query paged journal facts. Snapshot persistence envelope v2 names its exact covered sequence and cumulative history digest and contains the independently versioned runtime projection payload v3, serialized from the live-compacted projection without first cloning a historical projection. Invalid, mismatched, corrupt, envelope-v1, payload-v1/v2, or otherwise unsupported optional checkpoints are discarded and replayed rather than guessed or migrated.
