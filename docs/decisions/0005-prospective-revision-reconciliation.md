...
After the immediate revision pin is durable, the active projection retains only the current pin and latest reconciliation request/plan summary. Each live execution carries its immutable governing revision directly. Once a completed occurrence's transition obligations close, its bounded current scope/node summary preserves that revision, creation boundary, terminal state, attempt count/latest identity, conservative side-effect classification, and retained outputs. Reconciliation reads those execution-owned facts and validates the current exact pin/request boundary; it does not reconstruct an arbitrary historical revision from compacted pins. Superseded executions, plans, decisions, and old pins remain exact journal history and are obtained through paged historical queries; their removal from active state cannot make completed effects rewritable.
