...
Proposal validation and streaming rebuild use the logical union of both tiers. Security audit, peer execution, runtime events/snapshots, and artifacts retain separate policies. Daemon configuration schema 5 names `application_receipts.hot_receipt_bound`, `application_receipts.archive_batch_size`, and `security_audit_record_bound` independently. Redb physical schema 6 is exact-current and older stores are refused; no migration is claimed.
