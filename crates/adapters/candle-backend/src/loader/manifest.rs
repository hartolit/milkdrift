//! Bounded Safetensors header inspection and retained source manifest.
...
use domain_contracts::{
    BackendId, CapacityExhausted, CapacityResource, LoadError, ScalarType, ScalarTypeSet,
};
...
use crate::failure::{
    CODE_DUPLICATE_TENSOR, CODE_HEADER_ALLOCATION, CODE_HEADER_BOUNDS, CODE_HEADER_DECODE,
    CODE_HEADER_LIMIT, CODE_INSPECTION_ALLOCATION, CODE_INSPECTION_INVENTORY_LIMIT,
    CODE_METADATA_LIMIT, CODE_NUMERIC_OVERFLOW, CODE_SOURCE_IDENTITY_LENGTH, CODE_TENSOR_LIMIT,
...
impl<'a> InspectionBudget<'a> {
    const fn new(limits: &'a InspectionLimits) -> Self {
        Self {
            limits,
            tensor_count: 0,
    ...
    fn add_inventory(&mut self, backend: BackendId, bytes: usize) -> Result<(), LoadError> {
...
fn map_parse_failure(backend: BackendId, failure_kind: Option<ParseFailure>) -> LoadError {
    match failure_kind {
...
mod tests {
    ...
    use super::{
        ...
        inspect_weight_shards, map_parse_failure, parse_header,
    };
    use crate::failure::{
