...
use milkdrift_peer_protocol::{
    ...
    PeerInvocationRequest, PeerObservation, PeerRequestId,
};
use serde::{Deserialize, Serialize};

use crate::{PageSize, PersistenceError, TimestampMillis, WorkerId};

/// Current checksummed peer-execution primary-record schema.
...
/// Explicit retention/archival state. Archival never removes the idempotency tombstone.
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(rename_all = "snake_case", tag = "type", deny_unknown_fields)]
pub enum PeerExecutionRetention {
    /// Complete bounded observations remain in the live store.
    Retained,
    /// The terminal record is archived under explicit policy.
    Archived {
        /// Archive boundary.
...
pub struct PeerExecutionRecord {
    ...
    pub retention: PeerExecutionRetention,
...
pub struct PeerAdmission<'a> {
    ...
    /// Total retained/archived record ceiling; reaching it refuses new identities.
...
pub struct PeerObservationPage {
    ...
    pub record: PeerExecutionRecord,
