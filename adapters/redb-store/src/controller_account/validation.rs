//! Independent journal/event/lineage checks before any controller mutation.
use super::{account_in_transaction, decode_account, require_account_in_read};
use crate::{error, schema::CONTROLLER_ACCOUNTS, schema::CONTROLLER_RUN_BINDINGS};
use milkdrift_persistence::{
    AtomicRunCommitRequest, AttemptUsage, ControllerAccountAction, ControllerAccountId,
    ControllerAccountTransaction, ControllerAdmissionOutcome, ControllerAssessmentBoundary,
    ControllerReservationId, CurrencyCode, PersistenceError, RunEventEnvelope, RunEventKind,
};
use redb::WriteTransaction;

pub(super) fn validate_event_transaction_contract(
...
pub(super) fn validate_lineage_transaction_contract(
    write: &WriteTransaction,
    request: &AtomicRunCommitRequest,
    binding: Option<&ControllerAccountId>,
) -> Result<(), PersistenceError> {
