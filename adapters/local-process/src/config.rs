...
use milkdrift_capability::{
    ...
    CapabilityDescriptor, CapabilityId, DescriptorBuilder, ExtensionKey, FeatureContract,
    FeatureId, IdempotencyBehavior, Locality, OperationContract, OperationId, ProviderProfileRef,
...
/// Exact current process-profile document schema.
...
pub enum ProcessProfileError {
    ...
    #[error("unsupported process profile schema version {found}; supported version is 1")]
    UnsupportedVersion {
        /// Observed version.
        found: u32,
    },
...
impl<'de> Deserialize<'de> for ProcessProfileDocument {
        ...
        D: Deserializer<'de>,
    {
        ...
        if wire.schema_version != PROCESS_PROFILE_SCHEMA_VERSION_V1 {
...
impl ProcessProfileDocument {
    /// Parses a duplicate-safe, size-bounded exact-v1 profile document.
    pub fn from_json(bytes: &[u8]) -> Result<Self, ProcessProfileError> {
        ...
        if version != PROCESS_PROFILE_SCHEMA_VERSION_V1 {
...
impl ProcessProfile {
    ...
    pub fn descriptor(&self) -> Result<CapabilityDescriptor, ProcessProfileError> {
        let schema_value = BoundedJson::new(json!({
            "type": "object",
            "additionalProperties": true,
            "x-milkdrift-process-profile-schema": PROCESS_PROFILE_SCHEMA_VERSION_V1
        }))
        ...
        let extension_value = BoundedJson::new(json!({
            "schema_version": PROCESS_PROFILE_SCHEMA_VERSION_V1,
        ...
        .labels(BTreeSet::from(["local process".to_owned()]))
    ...
    fn validate(&self) -> Result<(), ProcessProfileError> {
        ...
        if self.revision == 0 || self.descriptor_revision == 0 || self.max_concurrent == 0 {
            return Err(ProcessProfileError::Invalid(
                "profile, descriptor, and admission revisions/counts must be nonzero".to_owned(),
            ));
        }
