...
fn run(arguments: Arguments) -> EvidenceResult {
    ...
    let report = json!({
        "explicit_continuation": if arguments.mode == Mode::Deterministic { Some(continuation::scenario(&arguments, &output)?) } else { None },
...
mod acceptance;
#[path = "local-model-evidence/continuation.rs"]
mod continuation;
